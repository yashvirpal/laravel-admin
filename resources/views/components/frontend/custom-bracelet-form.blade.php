<div class="customised-form">
    <h4>For Energize, We require these details</h4>
    <form>
        <div class="row">
            <div class="col-md-6">
                <div class="input-group">
                    <label for="cb_name">Name</label>
                    <input type="text" name="cb_name" id="cb_name" value="" />
                </div>
            </div>
            <div class="col-md-6">
                <div class="input-group">
                    <label for="cb_dob">Date Of Birth</label>
                    <input type="date" name="cb_dob" id="cb_dob" value="" />
                </div>
            </div>
            <div class="col-md-6">
                <div class="input-group">
                    <label for="cb_problem">Problem</label>
                    <input type="text" name="cb_problem" id="cb_problem" value="" />
                </div>
            </div>
            <div class="col-md-6">
                <div class="input-group">
                    <label for="cb_specify_problem">Specific Problem</label>
                    <textarea name="cb_specify_problem" id="cb_specify_problem"></textarea>
                </div>
            </div>
        </div>
    </form>
</div>